...
export function runForFiles(inputFileNames: string[], compilerOptions: ts.CompilerOptions) {
	...
	return {
		documents: inputFileNames.reduce<tsdoc.DocComment[]>(
			(all, inputFileName) => all.concat(runForFile(program, tsdocParser, inputFileName)),
			[]
		),
...
function runForFile(program: ts.Program, tsdocParser: tsdoc.TSDocParser, inputFileName: string) {
